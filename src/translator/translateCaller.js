...
import { getCachedTranslation, setCachedTranslation } from "/src/util/translate_cache.js";
...
async function getTranslate(text, sourceLang, targetLang, engine) {
  ...
  var cached = await getCachedTranslation(text, sourceLang, targetLang, engine);
  if (cached) return cached;

  // 2. Call translate API
  var result = await translator[engine]?.translate(text, sourceLang, targetLang);

  // 3. Save to persistent cache if successful
  if (result?.targetText) {
    setCachedTranslation(text, sourceLang, targetLang, engine, result);
  }
