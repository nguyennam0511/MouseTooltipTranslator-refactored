...
export default class Youtube extends BaseVideo {
  ...
  static getPlayerApi() {
    return document.querySelector(this.playerApiSelector);
  }
  static reloadCaption() {
    var ele = this.getPlayerApi();
    if (ele && typeof ele.setOption === "function") {
      ele.setOption("captions", "reload", true);
    }
  }
  static loadCaption() {
    var ele = this.getPlayerApi();
    if (ele && typeof ele.loadModule === "function") {
      ele.loadModule("captions");
    }
  }
  static unloadCaption() {
    var ele = this.getPlayerApi();
    if (ele && typeof ele.unloadModule === "function") {
  ...
  static async handleCaptionOnOff(e) {
    if (e?.code == "KeyC" || e?.button == 0) {
      ...
      if (btn) {
        this.setting["subtitleButtonToggle"] = btn.getAttribute("aria-pressed");
        this.setting.save();
      }
    }
  }
