...
function loadScript(name, htmlPath) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    iFrame.name = name;
    iFrame.id = name;
    iFrame.src = util.getUrlExt(htmlPath);
    var css = name.includes("Debug") ? debugCSS : iFrameCSS;
...
function addTooltipBox(img, textBox, text, targetLang) {
  ...
  tooltipContent.dir = getRtlDir(targetLang);

  const tooltipTarget = document.createElement("div");
  Object.assign(tooltipTarget.style, {
    position: "absolute",
    left: `${left}px`,
  ...
  img.parentElement.appendChild(tooltipTarget);
  ...
  tooltipContent.addEventListener("mouseenter", () => {
    ...
    tooltipContent.style.opacity = 0.0; // Reduce opacity
  });
  ...
  tooltipContent.addEventListener("mouseleave", () => {
    ...
    tooltipContent.style.opacity = 1; // Restore opacity
  });
...
async function createOcrTextBlock(img, textBox, color) {
  ...
  div.className = "ocr_text_div notranslate";
  div.style.border = "2px solid " + color;
  div.textContent = textBox["text"];
  img.parentElement.appendChild(div);

  // change z-index
  var zIndex = Math.max(0, 100000 - getBboxSize(textBox["bbox"])) + textBox["confidence"];
...
function removeOcrBlock() {
  document.querySelectorAll(".ocr_text_div").forEach(el => el.remove());
}
